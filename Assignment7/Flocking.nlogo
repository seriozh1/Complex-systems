globals [
  last-average-heading
  last-average-position
  stable-ticks
]
...
to setup
  ...
  set last-average-heading 0
  set last-average-position (list 0 0)
  set stable-ticks 0
  reset-ticks
end
...
to check-stability
  let current-average-heading mean [heading] of turtles
  let current-average-x mean [xcor] of turtles
  let current-average-y mean [ycor] of turtles
  let current-average-position (list current-average-x current-average-y)

  let last-average-x first last-average-position
  let last-average-y last last-average-position
  let distance-moved sqrt ((current-average-x - last-average-x) ^ 2 + (current-average-y - last-average-y) ^ 2)

  ifelse (abs (subtract-headings current-average-heading last-average-heading) < 5) and
     (distance-moved < 1)
    [ set stable-ticks stable-ticks + 1 ]
    [ set stable-ticks 0 ]

  set last-average-heading current-average-heading
  set last-average-position current-average-position
end
