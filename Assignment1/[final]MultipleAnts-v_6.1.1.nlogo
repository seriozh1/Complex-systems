...
to setup
  ...
  set list-max-step-size []
  set list-max-turn-angle []
  set list-ticks []
...
to update-color
  set color (ifelse-value
    food-eaten >= 0 and food-eaten <= 2 [ red ]
    food-eaten > 2 and food-eaten <= 4 [ blue ]
    food-eaten > 4 [ yellow ])
end

to go
  if run-count > repeat-N [stop]
  if not any? patches with [pcolor = green] [restart]
  ask turtles
  [
