...
globals [
  list-population-size
  list-diffusion-rate
  list-evaporation-rate
  list-ticks
  cumulative-ticks
  run-count
  repeat-N
...
to restart
  set cumulative-ticks cumulative-ticks + ticks
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Automatic experiments procedures ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

to run-experiments
  set list-diffusion-rate []
  set list-evaporation-rate []
  set list-population-size []
  set list-ticks []

  let i 1
  while [i <= 99] [
    set list-diffusion-rate lput i list-diffusion-rate
    set list-evaporation-rate lput i list-evaporation-rate
    set i i + 1
  ]

  foreach list-diffusion-rate [
    d ->
    reset-timer
    print word "d = " d
    foreach list-evaporation-rate [
      ev ->
      print word "    ev = " ev
      set diffusion-rate d
      set evaporation-rate ev
      setup
      run-model 20000
...
to go  ;; forever button
  if all? patches [food = 0] and all? turtles [not carrying-food?] [restart]
  ask turtles [
    move
    recolor
  ]
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="true">
    <preExperiment>pre-setup</preExperiment>
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <steppedValueSet variable="evaporation-rate" first="1" step="1" last="10"/>
