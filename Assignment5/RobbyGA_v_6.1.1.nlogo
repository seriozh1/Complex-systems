...
to update-allele-distribution
  ; Assuming the individual's chromosome is already set, update its allele-distribution
  set allele-distribution map [ action ->
    occurrences action chromosome
  ] ["move-north" "move-east" "move-south" "move-west" "move-random" "stay-put" "pick-up-can"]
end
...
to create-next-generation ;[best-individual]
  ...
  repeat crossover-count [
    ...
    ifelse random-float 1 < crossover-probability [
      ...
      create-children parent1 parent2 child-chromosomes
    ] [
...
to create-children [parent1 parent2 child-chromosomes]
  ; Assuming child-chromosomes is a list containing two chromosomes
  let child-chromosome1 item 0 child-chromosomes
  let child-chromosome2 item 1 child-chromosomes

  ; Create first child with first chromosome
  ask parent1 [
    hatch 1 [
      rt random 360 fd random-float 3.0
      set chromosome child-chromosome1
      ; Ensure allele-distribution or other properties are also updated if necessary
      update-allele-distribution
    ]
  ]

  ; Create second child with second chromosome
  ask parent2 [
    hatch 1 [
      rt random 360 fd random-float 3.0
      set chromosome child-chromosome2
      ; Ensure allele-distribution or other properties are also updated if necessary
      update-allele-distribution
    ]
  ]
...
to clone-parent [parent]
  ask parent [
    hatch 1 [
      rt random 360 fd random-float 3.0
      set chromosome [chromosome] of parent
      ; Ensure allele-distribution or other properties are also cloned if necessary
    ]
  ]
end
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="crossover-probability">
      <value value="0"/>
      <value value="0.3"/>
      <value value="0.5"/>
      <value value="0.7"/>
