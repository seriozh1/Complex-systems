...
to create-next-generation
  ...
  if selection-method = "Fitness-Proportionate" [
    let total-fitness sum [fitness] of individuals
    repeat population-size [
      let pick random-float total-fitness
      let running-total 0
      foreach sort individuals [ind ->
        set running-total running-total + [fitness] of ind
        if running-total >= pick [
          set new-genomes lput [chromosome] of ind new-genomes
          stop
        ]
      ]
    ]
  ...
  if selection-method = "Elitism" [
    let best-genome [chromosome] of max-one-of individuals [fitness]
    set new-genomes lput best-genome new-genomes
    repeat population-size - 1 [
      let contenders n-of tournament-size individuals
      let winner [chromosome] of max-one-of contenders [fitness]
      set new-genomes lput winner new-genomes
    ]
  ]

  if selection-method = "Linear Rank" [
    let ranked-individuals sort-on [fitness] individuals
    let total-ranks ((population-size * (population-size + 1)) / 2)
    repeat population-size [
      let pick random-float total-ranks
      let running-total 0
      let rank 1
      foreach ranked-individuals [ind ->
        set running-total running-total + rank
        if running-total >= pick [
          set new-genomes lput [chromosome] of ind new-genomes
          stop
        ]
  ...
  foreach new-genomes [genome ->
    create-individuals 1 [
      set chromosome genome
      let actions ["move-north" "move-east" "move-south" "move-west" "move-random" "stay-put" "pick-up-can"]
      set allele-distribution map [action -> occurrences action genome] actions
    ]
  ]
end


to tournament-selection [num-individuals]
  repeat num-individuals [
    let contenders n-of tournament-size individuals
    let winner max-one-of contenders [fitness]
    ask winner [ hatch 1 [ set color [color] of myself set chromosome [chromosome] of myself ] ]
  ]
end
...
selection-method
selection-method
"Fitness-Proportionate" "Elitism" "Linear Rank" "Tournament"
1
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="false">
    ...
    <exitCondition>ticks = number-of-generations</exitCondition>
    <metric>best-fitness</metric>
    <runMetricsCondition>ticks = number-of-generations</runMetricsCondition>
    ...
    <enumeratedValueSet variable="selection-method">
      <value value="&quot;Fitness-Proportionate&quot;"/>
      <value value="&quot;Elitism&quot;"/>
      <value value="&quot;Linear Rank&quot;"/>
